...
from typing import Union, Mapping, Any, Sequence, Iterable, Optional
...
def is_matched_to_file_name_glob_pattern(file_path: Path, glob_pattern: Optional[str] = None) -> bool:
    """
    ファイルパスのファイル名が、指定されたGlobパターンにマッチするかどうか
    Args:
        file_path(pathlib.Path): ファイルパス
        glob_pattern(Optional[str]): Globパターン。Noneなら、ファイル名に関わらずマッチしたとみなす。 (default: None)
    Returns:
        マッチするかどうか
    """
    if glob_pattern is None:
        return True
    file_names_matched: Sequence[str] = [file.name for file in file_path.parent.glob(glob_pattern)]
    return file_path.name in file_names_matched


def gzipped_bytes(file: Path) -> bytes:
    """
    ファイルのgzip圧縮バイト列
    Args:
        file(pathlib.Path): ファイル
    Returns:
        gzip圧縮バイト列(bytes)
    """
